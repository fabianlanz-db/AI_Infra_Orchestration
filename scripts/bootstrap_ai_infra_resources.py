...
Creates/updates:
- UC schema and synthetic Delta tables in fl_demos.ai_infra_orchestration_demo
...
VS_ENDPOINT = "ai_infra_orchestration_vs_ep"
VS_INDEX = f"{CATALOG}.{SCHEMA}.ai_kb_index"
SOURCE_TABLE = f"{CATALOG}.{SCHEMA}.ai_kb_chunks"
LAKEBASE_PROJECT_ID = "ai-infra-orchestration-db"


DDL_AND_DATA = f"""
CREATE SCHEMA IF NOT EXISTS {CATALOG}.{SCHEMA};

CREATE OR REPLACE TABLE {CATALOG}.{SCHEMA}.ai_assets USING DELTA AS
SELECT
...
CREATE OR REPLACE TABLE {CATALOG}.{SCHEMA}.ai_kb_chunks USING DELTA AS
SELECT
  ...
  element_at(array('Optics','WaferHandling','VacuumSystem','ThermalControl','LaserSubsystem'), cast(rand() * 5 as int) + 1) AS subsystem,
  concat('AI infrastructure knowledge chunk ', cast(id as string), ' describing diagnostics, probable root cause, stepwise mitigation, safety constraints, and part replacement guidance for high-precision industrial operations.') AS content,
  concat_ws(',', 'ai-infra', 'agent-framework', 'ops-demo') AS tags,
...
def main() -> None:
    ...
    operation = workspace.postgres.create_project(
        project=Project(spec=ProjectSpec(display_name="AI Infra Orchestration DB", pg_version="17")),
